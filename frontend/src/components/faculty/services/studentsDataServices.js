...
export const Fetchlink = async (email, mid, fullcourse) => {
  ...
  var studentTopic = "";
  let data = [];
  export async function getFastData(subject, ismid1, ismid2, sEmail, sem, acadYear){
  ...
  let arr = [];
  export async function getDataData(students,subject,ismid1,ismid2,sem,acadYear){
    for(let i=0; i<students.length; i++){
      arr.push(getFastData(subject,ismid1,ismid2,students[i],sem,acadYear));
    }
  ...
  export async function getAllStudents(
    ...
    data = [];
    await getDataData(students,subject,ismid1,ismid2, sem, acadYear);
    return {
      data: data,
      student: student,
